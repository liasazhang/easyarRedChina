...
import cn.easyar.samples.helloar.activity.FamousShowActivity;
...
public class HelloAR
{
    ...
    public HelloAR(Activity context)
    {
    ...
    private void loadFromJson(ImageTracker tracker, String jsonPath)
    {
        ...
        InputStream s = null;
        try {
            s = mContext.getAssets().open(jsonPath);
...
//        可以加图片到本地
        loadFromImage(tracker, "飞夺泸定桥.jpg", "飞夺泸定桥");
        loadFromImage(tracker, "过雪山.jpg", "过雪山");
        loadFromImage(tracker, "攻占总统府.jpg", "攻占总统府");
        loadFromImage(tracker, "四渡赤水.jpg", "四渡赤水");
        loadFromImage(tracker, "吴起镇会师.jpg", "吴起镇会师");
                ...
                if (result != null) {
                    for (TargetInstance targetInstance : result.targetInstances()) {
                        ...
                        if (status == TargetStatus.Tracked && Constants.flag < 1) {
                             ...
                             */
                            if (Constants.flag == 1) {
                                final String fileName = target.name();
                                Constants.b_status1 = 1;
                                mContext.runOnUiThread(new Runnable() {
                                    @Override
                                    public void run() {
                                        Log.e("fileName",fileName);
                                        Toast.makeText(mContext, "页面正确", Toast.LENGTH_SHORT).show();
                                        Constants.b_status1 = 1;
                                        Intent intent = new Intent(mContext, FamousShowActivity.class);
